...
/* Row/Col pins â€” Rows = PA0..PA3 (Output), Cols = PA4..PA7 (Input PU) */
...
static void calc_reset(void)
{
  ...
  HD44780_Clear();
  HD44780_SetCursor(0,0); HD44780_PrintStr("San sang!");
  HD44780_SetCursor(0,1); HD44780_PrintStr("Tinh: ");
}
...
static void lcd_show_expr(void)
{
  ...
  snprintf(line, sizeof(line), "Tinh: %-10s", expr);
  HD44780_PrintStr(line);
}

static void lcd_show_result(long v)
{
  char buf[17]; snprintf(buf, sizeof(buf), "Ket qua: %-10ld", v);
...
int main(void)
{
  ...
  /* Ä‘áº£m báº£o hÃ ng á»Ÿ má»©c HIGH khi idle */
  ...
  while (1)
  {
    ...
    if (key) {
      /* Hiá»‡n phÃ­m vá»«a báº¥m á»Ÿ cuá»‘i dÃ²ng 2 (debug) */
